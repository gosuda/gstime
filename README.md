...
- A failed round changes a valid anchor from `SYNCED` to `HOLDOVER`. Its deadline is the earlier of the existing `MaxAgeNs` horizon and `LastSuccessfulRoundRaw + MaxHoldoverAgeNs`; failed polls never renew it. A zero holdover limit preserves the existing anchor horizon. Expiry or a detected continuity fault yields `DESYNC`. A fresh valid round can recover synchronization.
...
- NTS authenticates the server and packets; it does not establish the server's time accuracy or failure-domain independence. The KE reader requires a complete validated End-of-Message, bounded records (16 KiB), and a bounded message (64 KiB). It honors the negotiated NTP host/UDP port and requires TLS 1.3 with `ntske/1` ALPN.
